...
RayTracingRenderer::RayTracingRenderer(std::filesystem::path root_dir) {
  ...
  //   0.2, 0.5)); auto material_left = std::make_shared<dielectric>(1.5); auto
  //   material_right = std::make_shared<metal>(color(0.8, 0.6, 0.2), 0.0);
  ...
  //   m_world.add(make_shared<sphere>(point3(-1.0, 0.0, -1.0), 0.5,
  //   material_left)); m_world.add(
  //       make_shared<sphere>(point3(-1.0, 0.0, -1.0), -0.45, material_left));
  //   m_world.add(make_shared<sphere>(point3(1.0, 0.0, -1.0), 0.5,
  //   material_right)); m_world.add(make_shared<sphere>(point3(1.0, 0.0, -1.0),
  //   100, material_left));
}
...
void RayTracingRenderer::render_frame() {
  ...
  //   m_image_height); Log::the().add_log("Threads=%zu\n", m_n_threads);
  ...
  //     int max_depth;
  //   };
  //   RenderInfo ri{m_image_height, m_image_width, samples_per_pixel,
  //   max_depth}; auto render_task = [](hittable_list const world, camera const
  //   cam,
  ...
  //     m_futures.emplace_back(std::async(std::launch::async, render_task,
  //     m_world,
  //                                       cam, ri, start, stop));
  //     cur_step += step;
  //   }
